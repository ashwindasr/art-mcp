...
builds_schema_fields = {
    'name', 'group', 'version', 'release', 'assembly', 'el_target', 'arches',
    'installed_packages', 'parent_images', 'source_repo', 'commitish', 
    'rebase_repo_url', 'rebase_commitish', 'embargoed', 'start_time', 
    'end_time', 'artifact_type', 'engine', 'image_pullspec', 'image_tag',
    'outcome', 'art_job_url', 'build_pipeline_url', 'pipeline_commit',
    'schema_level', 'ingestion_time', 'record_id', 'build_id', 'nvr',
    'rmetic', 'installed_rpms', 'build_component', 'build_priority'
}

bundles_schema_fields = {
    'name', 'group', 'version', 'release', 'assembly', 'source_repo', 'commitish',
    'rebase_repo_url', 'rebase_commitish', 'start_time', 'end_time', 'engine',
    'image_pullspec', 'image_tag', 'outcome', 'art_job_url', 'build_pipeline_url',
    'pipeline_commit', 'schema_level', 'ingestion_time', 'operand_nvrs', 
    'operator_nvr', 'record_id', 'build_id', 'nvr', 'bundle_package_name',
    'bundle_csv_name', 'build_component', 'build_priority'
}
...
fbcs_schema_fields = {
    ...
    'rebase_repo_url', 'rebase_commitish', 'start_time', 'end_time', 'engine',
    'image_pullspec', 'image_tag', 'outcome', 'art_job_url', 'build_pipeline_url',
    'pipeline_commit', 'schema_level', 'ingestion_time', 'bundle_nvrs', 'arches',
    'record_id', 'build_id', 'nvr', 'build_component', 'build_priority'
}

taskruns_schema_fields = {
    'creation_time', 'build_id', 'task', 'task_run', 'task_run_uid', 'pipeline_run',
    'pipeline_run_uid', 'pod_name', 'pod_phase', 'scheduled_time', 'initialized_time',
    'start_time', 'containers', 'max_finished_time', 'success', 'record_id'
}
...
async def get_failed_builds(
    days_back: int = 7,
    failure_type: str = "FAILED",
    limit: int = 25
) -> str:
    """Get details about failed builds for debugging and analysis."""
    where_clause = f"outcome = '{failure_type}' AND start_time >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL {days_back} DAY)"
...
@mcp.tool()
async def search_related_builds(
    operator_nvr: str = "",
    bundle_nvr: str = "",
    search_depth: str = "full"
) -> str:
